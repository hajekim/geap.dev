...
> [!NOTE]
> **실무 시나리오: 글로벌 프리미엄 세단 마케팅 트렌드 분석 및 세션 협업**  
> 글로벌 마케팅 베테랑의 관점으로 북미 및 유럽 시장 환경을 분석하고, 도출된 분석 리포트를 팀원들과 공유 링크로 협업합니다.
...
```markdown
[Role] 너는 10년 차 글로벌 프리미엄 브랜드 마케팅 수석 전문가야.
[Context] 당사의 차세대 프리미엄 세단 글로벌 런칭을 앞두고 북미 및 유럽 시장의 최신 마케팅 경쟁 지형을 분석 중이야.
[Action] 최근 북미 및 유럽 시장의 '프리미엄 세단' 최신 트렌드를 검색하여 요약해 줘. 특히 주요 경쟁사들의 최근 마케팅 소구점(Selling Point)을 도출하고, 한국 자동차 브랜드가 차별화할 수 있는 핵심 전략 인사이트를 제시해 줘.
[Format] 1. 시장 트렌드 3줄 요약, 2. 경쟁사별 소구점 비교 분석표, 3. 전략적 액션 아이템 순으로 일목요연하게 정리해 줘.
```

| 쿼리 결과 화면 | Source 출처 검증 화면 |
| :---: | :---: |
| ![프리미엄 세단 트렌드 쿼리 결과](./img/06.webp) | ![Source 출처 검증 화면](./img/07.webp) |
...
| :---: | :---: |
| ![대화 세션 공유 인터페이스](./img/09.webp) | ![공유 링크 복사](./img/10.webp) |
...
> [!NOTE]
> **실무 시나리오: 글로벌 규제 정책 SWOT 분석 및 최신 기술 동향 파악**  
> 실시간 구글 검색을 통해 최신 규제 및 전시회 동향을 수집하고, Public URL 페이지를 직접 바인딩하여 분석합니다.

### 1) 실전 실습 1: EU 전기차 정책 변화 및 글로벌 SWOT 수립

```markdown
[Context] 2026년 현재 유럽연합(EU)의 친환경 정책 및 보조금 규제 변화가 급변하고 있습니다.
[Action] EU의 전기차(EV) 보조금 정책 변화와 탄소국경조정제도(CBAM) 관련 최신 글로벌 뉴스를 실시간 검색해 줘. 이러한 정책 변화가 한국 자동차 제조사의 유럽 시장 진출에 미칠 영향을 분석해 줘.
[Format] SWOT(강점, 약점, 기회, 위협) 분석 매트릭스 표 형태로 시각화하여 정리하고, 최우선 대응 전략 2가지를 제시해 줘.
```

---

### 2) 실전 실습 2: CES 기술 동향 기반 피지컬 AI 경쟁 대응 전략

```markdown
[Role] 너는 테크 산업 분석 수석 애널리스트야.
[Action] 최근 열린 'CES 2026'에서 글로벌 선도 기업들이 발표한 피지컬 AI(Physical AI) 및 온디바이스 기술 마케팅 트렌드를 웹 검색으로 분석해 줘.
[Format] 1. 3대 핵심 기술 트렌드 요약, 2. 주요 경쟁사 추격 양상 분석, 3. 시장 방어를 위한 차별화 마케팅 핵심 메시지(3줄).
```
...
**Nano Banana**(이미지)와 **Veo**(동영상) 모델을 활용하여 비즈니스 포스터, 아키텍처 다이어그램, 시네마틱 홍보 영상을 직접 제작합니다.

> [!NOTE]
> **시각화 실습: 비즈니스 이미지 및 시네마틱 루프 영상 제작**  
> 텍스트 프롬프트와 참조 이미지를 결합하여 기업용 광고, 클라우드 아키텍처 다이어그램 및 광고 영상을 생성합니다.

### 1) 실전 실습 1: Nano Banana 모델을 활용한 비즈니스 이미지 생성
...
비디오 생성 도구 모드에서 프롬프트를 실행하여 고품질 루프 영상을 생성합니다.
...
Gemini Enterprise Canvas를 활용하여 아키텍처 요구사항을 바탕으로 구글 슬라이드 장표를 자동 생성합니다.

> [!NOTE]
> **Canvas 실습: 구글 슬라이드 자동 생성 및 디자인 톤앤매너 커스터마이징**  
> 요구사항 텍스트를 기반으로 슬라이드 장표를 자동 빌딩하고, 전문 기술문서 및 기업 브랜드 템플릿으로 확장합니다.

### 1) 실전 실습 1: GCP 백엔드 아키텍처 구글 슬라이드 생성
...
```markdown
[Context] 당사는 2027년 탄소 중립 규제 도입에 발맞춰 사내 물류 인프라를 친환경 스마트 모빌리티로 전환하는 프로젝트를 추진 중입니다. 목적은 경영진에게 전사 ESG 전환의 실질적인 투자 타당성을 제안하는 것입니다.
[Role] 당신은 친환경 인프라 설계 분야에서 15년 이상의 경력을 가진 ESG 전략본부 수석 전략 기획 전문가입니다.
[Action] 경영진 보고용 '친환경 모빌리티 인프라 전환 추진 기획서' 초안을 수립해 주세요. 사내 유휴 공간을 임직원 복합 친환경 커뮤니티 공간으로 연계하는 창의적인 아이디어가 포함되어야 합니다.
[Format] 1. 사업 개요 및 필요성, 2. 단계별 실행 프로세스, 3. 정량적 탄소 배출 저감 및 예산 예측 모델(표), 4. 임직원 캠페인 방안으로 마크다운 보고서로 작성하세요.
[Tone] 이사회 및 C-Level 대상이므로 비즈니스 가치와 비용 효율성에 초점을 둔 객관적이고 세련된 경영 비즈니스 톤을 유지하세요.
```

#### 시나리오 B. 고객 클레임 사과문 및 보상 안내

```markdown
[Context] 당사 원격 협업 서비스 'SpaceWork'의 서버 통신 오류로 인해 월요일 오전 9시부터 11시까지 2시간 동안 전사 접속 지연 장애가 발생하여 고객 클레임이 급증한 상황입니다.
[Role] 당신은 대고객 서비스 복구 및 평판 복원 기획 업무를 12년간 전담해 온 글로벌 CS 및 브랜드 홍보 총괄 실장입니다.
[Action] 유료 구독 고객들에게 발송할 '정중하고 진정성 있는 공식 사과문 및 보상 대책 안내 이메일' 초안을 기획하세요.
[Format] 메일 제목 3종 옵션 / 도입부(상황 투명 공유) / 사과 및 공감 / 구체적 보상안(요금 감면 등) / 재발 방지 대책 구조로 작성하세요.
[Tone] 어설픈 변명은 배제하고, 불편에 깊이 공감하는 정중한 사과의 톤과 함께 시스템을 통제하고 있다는 신뢰감을 전달하세요.
```

#### 시나리오 C. 보도자료 분석 및 SWOT 전략

```markdown
[Context] 경쟁사가 물류 창고 자동화용 차세대 자율주행로봇(AMR) 'FlexLoad'를 공개한 공식 보도자료를 배포했습니다. 당사 마케팅 및 제품 혁신팀이 취해야 할 대응 시나리오를 수립하는 것이 목적입니다.
[Role] 당신은 다국적 물류 자동화 기업에서 10년 이상 제품 기획 및 경쟁사 분석(CI)을 담당한 수석 마케팅 애널리스트입니다.
[Action] 보도자료 내용을 분석하여 당사 제품 및 마케팅 조직이 참고할 SWOT 경쟁 대응 보고서를 작성하세요.
[Format] 1. 3줄 핵심 팩트 요약, 2. 경쟁사 신모델 SWOT 매트릭스 표, 3. 당사 긴급 마케팅 차별화 전략 2종 제안.
[Tone] 주관적 추측을 배제하고 공학적 사양과 보도 팩트에 기반한 객관적 컨설턴트 톤을 유지하세요.
```

#### 시나리오 D. IT 장애 포스트모템 보고서

```markdown
[Context] 일요일 14:00~16:30 데이터베이스 스토리지 용량 부족(Full-Disk)으로 인해 자사 실시간 결제 트랜잭션 API 서비스 전체가 중단되는 장애가 발생했습니다.
[Role] 당신은 대용량 트랜잭션 환경에서 15년 이상의 DevOps 및 SRE 장애 대응을 총괄해 온 수석 사이트 안정성 엔지니어(Principal SRE)입니다.
[Action] 경영진 및 CISO에게 제출할 공식 '장애 사후 분석 및 인프라 이중화 재발방지대책(Post-Mortem)' 보고서를 작성하세요.
[Format] 1. 장애 메타 정보 및 타임라인 표, 2. 근본 원인 분석(RCA), 3. 단기 리스크 긴급 조치 내역, 4. 중장기 자동 디스크 스케일아웃 및 이중화 아키텍처 수립 계획.
[Tone] 간결하고 직관적인 개조식(~함, ~음) 표준 엔지니어링 기술 보고서 문체를 사용하세요.
```
